...
const teamData = [
	{
		...
		xHandle: "@cikoart",
		igHandle: "@ciko.draws",
		avatarSrc: "https://placehold.co/400x400/1a1a1a/bd5582?text=Ciko",
	},
	{
		name: "Yeenki",
		...
		xHandle: "@yeenkiink",
		igHandle: "@yeenki.ink",
		avatarSrc: "https://placehold.co/400x400/bd5582/ffffff?text=Yeenki",
		isMain: true,
	},
	{
		name: "Milo",
		role: "Asset Specialist",
		xHandle: "@milodigitalart",
		igHandle: "@milo_dgtl",
		avatarSrc: "https://placehold.co/400x400/1a1a1a/ba7cd9?text=Milo",
	},
];

const DiscordIcon = ({ size = 20, color = 'currentColor' }) => (
	<svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 127.1 96.3">
		<path fill={color} d="M107.4 8.01A51.36 51.36 0 0 0 94.62 1.9c-2.05-.55-4.08-.2-5.9 1-3.6 2.4-7.23 4.8-10.87 7.21-1.35.9-2.22 2.3-2.9 3.82-3.8-1.57-7.7-2.46-11.66-2.47-3.95 0-7.85.89-11.64 2.44-.67-1.52-1.54-2.92-2.9-3.8-3.64-2.4-7.27-4.81-10.87-7.21-1.82-1.2-3.85-1.55-5.9-1-4.7 1.25-9.3 3.6-13.78 7.08-10.1 7.9-18.6 18.26-25.04 29.5-6.23 11.02-8.52 23.3-6.9 35.84 1.3 9.77 5.75 18.9 12.3 26.24 16.7 18.8 40 26.5 61.3 26.5s44.6-7.7 61.3-26.5c6.55-7.34 11-16.47 12.3-26.24 1.62-12.55-.67-24.83-6.9-35.85-6.44-11.24-14.93-21.6-25.04-29.5-4.48-3.48-9.08-5.83-13.78-7.08zM49.2 59.9c5.12 0 9.28-5.32 9.28-11.89s-4.16-11.89-9.28-11.89-9.28 5.32-9.28 11.89 4.16 11.89 9.28 11.89zM77.9 59.9c5.12 0 9.28-5.32 9.28-11.89s-4.16-11.89-9.28-11.89-9.28 5.32-9.28 11.89 4.16 11.89 9.28 11.89z"/>
	</svg>
);

const servicesData = [
	{ title: "Original Character (OC) & Fan Art", category: "OC & Fan Art" },
	{ title: "R/18+ Illustrations", category: "R/18+ Art" },
	{ title: "Character Design/Concept", category: "Character Design/Concept" },
...
const portfolioItemsData = [
	...
	{ id: 3, category: "Asset Illustrations", title: "Neon Club Vibe BG", src: "https://placehold.co/600x800/111/fff?text=Neon+Club", date: '2025-04-20', serial: 103, isFeatured: true },
	{ id: 4, category: "R/18+ Art", title: "Vampire Noble (P.H.)", src: "https://placehold.co/600x800/220000/bd5582?text=Vampire+Noble", date: '2025-07-01', serial: 108, isFeatured: true },
	{ id: 5, category: "OC & Fan Art", title: "Cyber Analyst Sketch", src: "https://placehold.co/600x800/000/ba7cd9?text=Cyber+Analyst", date: '2025-03-25', serial: 102, isFeatured: false },	
	{ id: 6, category: "Character Design/Concept", title: "Elf Concept Sheet", src: "https://placehold.co/600x800/111/bd5582?text=Elf+Concept", date: '2025-08-05', serial: 106, isFeatured: false },	
	{ id: 7, category: "Asset Illustrations", title: "Detail Close Up Prop", src: "https://placehold.co/600x800/111/fff?text=Prop+Detail", date: '2025-02-14', serial: 104, isFeatured: false },	
	{ id: 8, category: "R/18+ Art", title: "The Beanz Clone (P.H.)", src: "https://placehold.co/600x800/1e1e1e/bd5582?text=R/18+Art+Placeholder", date: '2025-09-12', serial: 107, isFeatured: false },
	{ id: 9, category: "Asset Illustrations", title: "Retro Pixel Hero Asset", src: "https://placehold.co/600x800/1e1e1e/ba7cd9?text=Pixel+Asset", date: '2025-01-01', serial: 109, isFeatured: false },
];
					...
					<div className="flex flex-wrap gap-2">
						...
						{categories.map((cat) => (
							<button
								...
								className={`px-4 py-1 text-xs font-bold uppercase rounded-full transition-all duration-200 ${
									selectedCategory === cat
										? 'bg-[#ba7cd9] text-black'
						...
						<div className="absolute right-0 mt-2 w-48 bg-neutral-800 border border-neutral-700 shadow-xl z-20 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-300 hidden group-hover:block group-focus-within:block">
							{["dateDesc", "dateAsc", "serialAsc"].map((key) => (
								<button
									...
									className={`w-full text-left px-4 py-2 text-sm hover:bg-[#bd5582] hover:text-white transition-colors flex items-center gap-2 ${sortBy === key ? 'bg-[#bd5582]/30 text-white' : 'text-neutral-300'}`}
								>
									{key === "serialAsc" ? <Monitor size={16} /> : <CalendarDays size={16} />}
									{getSortLabel(key)}
								</button>
							))}
						</div>
					...
					{filteredAndSortedItems.map((item) => (
						<div key={item.id} className="relative overflow-hidden rounded-lg shadow-xl border border-neutral-800 transition-transform duration-300 hover:scale-[1.03] group h-80">
							...
							<div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent p-4">
								<span className="text-[#ba7cd9] text-xs font-bold uppercase tracking-widest mb-1 block">{item.category}</span>
...
const TeamCard = ({ member }) => {
	...
	return (
		<div className={`flex flex-col items-center p-6 transition-all duration-500 hover:scale-[1.05] group rounded-xl ${member.isMain ? 'bg-neutral-800/50 border border-[#bd5582]' : 'bg-neutral-900/50 border border-neutral-800'}`}>
			<div className={`rounded-full overflow-hidden shadow-2xl ring-4 ring-offset-4 ring-offset-[#0a0a0a] ${member.isMain ? 'ring-[#bd5582]' : 'ring-[#ba7cd9]' } ${sizeClass} mb-6`}>
				<img
			...
			<h3 className={`font-display font-bold uppercase leading-tight text-white ${nameSizeClass}`}>
				{member.name}
			</h3>
			<p className="text-sm text-[#ba7cd9] uppercase tracking-widest mt-1 mb-4">{member.role}</p>
			<div className="flex gap-4">
				{member.igHandle && (
					<a href="#" className="text-neutral-500 hover:text-[#bd5582] transition-colors">
						<Instagram size={18} />
					</a>
				)}
				{member.xHandle && (
                    ...
                    <div className="lg:w-1/3 z-10">
                        ...
                        <div className="flex items-center max-w-sm">
                            <input 
                                ...
                                className="w-full bg-[#1e1e1e] text-neutral-300 px-4 py-3 rounded-l-lg border-none focus:ring-0 focus:outline-none placeholder-neutral-600 text-sm font-body"
                            />
                            <button className="bg-[#bd5582] hover:bg-[#a0466b] text-white p-3 rounded-r-lg transition-colors">
                                <ArrowRight size={20} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="hidden lg:flex absolute bottom-0 left-1/2 -translate-x-1/2 items-end justify-center pointer-events-none z-0">
                        ...
                        */}
                        <img 
                            ...
                            alt="Mascot" 
                            className="h-[300px] object-cover opacity-80"
...
export default function App() {
	...
	const socialLinks = [
		...
		{ name: "TikTok", url: "#", icon: <Video size={18} /> },
		{ name: "Pixiv", url: "#", icon: <Image size={18} /> },
		{ name: "Patreon", url: "#", icon: <Heart size={18} /> },
			...
			<div className="py-20 bg-[#121212] overflow-hidden border-y border-neutral-900">
				...
				<div className="relative w-full overflow-hidden whitespace-nowrap mb-6">
					<div className="flex animate-marquee-right">
						{marqueeItems1.map((item, index) => <MarqueeItem key={`row1-${index}`} item={item} />)}
					</div>
				</div>
				<div className="relative w-full overflow-hidden whitespace-nowrap">
					<div className="flex animate-marquee-left">
						{marqueeItems2.map((item, index) => <MarqueeItem key={`row2-${index}`} item={item} />)}
					</div>
				</div>
			</div>
		);
	...
	const MarqueeItem = ({ item }) => (
		<div className="inline-block w-[280px] h-[350px] mx-4 relative overflow-hidden rounded-xl shadow-xl border border-neutral-800 transition-transform duration-300 hover:scale-[1.02] flex-shrink-0 group">
			...
			<div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
				<span className="text-[#ba7cd9] text-xs font-bold uppercase tracking-widest mb-1 block whitespace-normal">{item.category}</span>
				<h4 className="font-display text-2xl font-bold text-white uppercase leading-tight whitespace-normal">{item.title}</h4>
			</div>
		</div>
	);
			...
			<nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-[#0a0a0a]/90 backdrop-blur-md py-4 shadow-lg border-b border-neutral-900' : 'bg-transparent py-6'}`}>
				<div className="container mx-auto px-6 flex justify-between items-center">
					<h1 className="font-display text-2xl font-bold uppercase cursor-pointer tracking-wider text-white" onClick={() => handleNavClick('home')}>
						YEENKI<span className="text-[#bd5582]">'</span>INK
					</h1>
				...
				<>
						...
						<div className="container mx-auto px-6 relative z-20 text-center flex flex-col items-center h-full justify-center pt-32 pb-20">
							...
							<div className="relative w-full max-w-4xl h-[60vh] md:h-[70vh] flex items-end justify-center">
								<img
									...
									alt="Character Illustration Showcase"
					...
					<section id="team" className="py-24 bg-neutral-900/30 border-y border-neutral-900">
						<div className="container mx-auto px-6">
							...
							<div className="flex flex-col items-center md:flex-row md:justify-center gap-8 md:gap-12">
							...
							<div className="grid md:grid-cols-2 gap-12">
								<div className="space-y-2">
									{services.map((service, index) => (
										<div	
											key={index}
											className={`p-6 border-l-2 cursor-pointer transition-all duration-300 rounded-tr-lg rounded-br-lg ${activeService === index ? 'border-[#bd5582] bg-neutral-900' : 'border-neutral-800 hover:bg-neutral-900/50'}`}
											onClick={() => setActiveService(index)}
										>
											<div className="flex items-center justify-between">
												<h3 className={`font-display text-2xl uppercase tracking-wide ${activeService === index ? 'text-white' : 'text-gray-500'}`}>
													{service.title}
												</h3>
												<span className={`${activeService === index ? 'text-[#ba7cd9]' : 'text-neutral-700'}`}>
													{service.icon}
												</span>
											</div>
										</div>
								...
								<div className="relative h-full min-h-[300px] flex flex-col justify-center p-8 bg-neutral-900/20 border border-neutral-800 rounded-xl shadow-2xl overflow-hidden">
									...
									<h3 className="font-display text-4xl text-white mb-6 uppercase leading-tight relative z-10">
										{services[activeService].title}
									</h3>
									<p className="font-body text-gray-400 text-lg leading-relaxed relative z-10">
								...
								{services.slice(0, 4).map((service, index) => {
									...
									return (
										<button	
											...
											className="group relative overflow-hidden bg-neutral-900 cursor-pointer h-[280px] flex items-end p-8 border border-neutral-800 hover:border-[#bd5582] transition-colors rounded-lg text-left"
										>
											<img	
												src={item.src}	
												alt={service.title}	
												className="absolute inset-0 w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 scale-100 group-hover:scale-105 opacity-40 group-hover:opacity-80"
											/>
											<div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent"></div>
											<div className="relative z-10 w-full">
												<span className="text-xs font-bold uppercase tracking-widest text-[#ba7cd9] mb-2 block">
													{service.category}
												</span>
												<h3 className="font-display text-3xl font-bold text-white uppercase leading-none mb-4">
													{service.title}
												</h3>
												<div className="h-0.5 w-8 bg-neutral-700 group-hover:bg-[#bd5582] group-hover:w-full transition-all duration-500"></div>
											</div>
										</button>
									)})}
							</div>
							...
							<div className="flex flex-col md:flex-row gap-16">
								<div className="md:w-1/2">
									...
									<div className="flex flex-col gap-6">
										<a	
											href="mailto:yeenkiink@gmail.com"	
											className="inline-flex items-center justify-center gap-3 bg-white text-black font-display font-bold text-lg uppercase px-10 py-5 hover:bg-[#bd5582] hover:text-white transition-all duration-300 w-full rounded-lg shadow-xl"
										>
											<Mail size={20} /> yeenkiink@gmail.com
										</a>
									...
									<div className="grid grid-cols-2 gap-4">
										{socialLinks.map((link, idx) => (
											<a	
												...
												target="_blank"
												rel="noopener noreferrer"
												className="flex items-center gap-3 p-4 border border-neutral-800 hover:border-[#bd5582] hover:bg-neutral-900 transition-all duration-300 group rounded-lg"
											>
												<span className="text-gray-500 group-hover:text-[#bd5582] transition-colors">{link.icon}</span>
												<span className="font-body text-sm font-medium text-gray-300 group-hover:text-white tracking-wide uppercase">{link.name}</span>
											</a>
										))}
									</div>
								</div>
